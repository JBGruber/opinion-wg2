...
prompt <- readr::read_file("1._prompt_v0.2.md")
schema <- list(
  type = "object",
  properties = list(
    category = list(
    ...
    tool_name_clean = list(
      type = "string",
      description = "Standardize the tool name (e.g., 'scikit learn' → 'scikit-learn', 'NLTK library' → 'NLTK')"
    ),
  ...
  required = c("category", "tool_name_clean", "opinion_mining_tool", "note")
)
