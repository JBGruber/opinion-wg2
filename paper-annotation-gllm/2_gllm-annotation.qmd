...
for (pdf in pdfs) {
  ...
  if (interactive()) cli::cli_alert_info("{.file {pdf}} annotated ({which(pdfs == pdf)}/{length(pdfs)})")
}
```
...
results <- file.path("results", pdfs) |> 
  setdiff(c("results/10.1007_978-981-99-3982-4_16.pdf")) |> 
  map(read_results) |> 
  bind_rows() |> 
...
```{r}
rio::export(results_corrected, "2._annotation-results.csv")
```
